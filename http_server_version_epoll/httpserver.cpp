...
{ 
    ...
    */
    ...
    /*-------------------------------------------------------------*/
    /* 创建epoll对象,返回epoll树的fd */
    this->epfd = epoll_create(MAX_EPOLL_SIZES);
    if(this->epfd == -1){
        std::cout<< TAG_ERROR <<"epoll create fault: "<< strerror(errno) <<std::endl;
        return errno;
    }
    /* 初始化evt结构体，对应的是listenfd，用于向epoll树中加入listenfd对应的event */
    this->evt.events = EPOLLIN;
    this->evt.data.fd = this->socket_fd;//listenfd

    if(epoll_ctl(this->epfd, EPOLL_CTL_ADD, this->socket_fd, &evt) == -1) //将listenfd加入epoll树中
    {
        std::cout<< TAG_ERROR << "epoll add error:"<< strerror(errno) <<std::endl;
        return errno;
    }
    this->isCreateEpoll = true;
    /*-------------------------------------------------------------*/

    //HttpInit Finish
    if(this->isBind && this->isListen && this->isCreateEpoll) {
        this->isInit = true;
        return 0;
    }
...
void myHttpServer::HttpServer::ExecReqst()
{
    ...
    {
        std::cout<< TAG_INFO << "Init Success!" << std::endl;
        std::cout<< TAG_INFO << "Waiting for clients..." << std::endl;
    }
    ...
    while (1)
    {
        ...
        /*
            ...
            param2: 从内核中得到的事件的集合
            param3: 
            param4：参数timeout是超时时间（毫秒，0会立即返回，-1将不确定，也有说法说是永久阻塞）

            return: 表示有多少个文件描述符被监控成功，被监控成功的文件描述符放在evts中;
                    该函数返回需要处理的事件数目，如返回0表示已超时。如果返回–1，则表示出现错误，需要检查 errno错误码判断错误类型。
        */
        this->readyfd = epoll_wait(this->epfd, evts, MAX_EPOLL_SIZES, -1);

        if(this->readyfd == -1){
            if(errno == EINTR) continue;
            std::cerr << TAG_ERROR << "Epoll fault:" << strerror(errno) <<std::endl;
            return ;
        }

        //遍历监控到的fd对应的evts数组
        for (int i = 0; i < this->readyfd; i++)
        {
            if(!(evts[i].events & EPOLLIN)) continue;

            if(evts[i].data.fd == this->socket_fd)
            { //输出的事件中，有listenfd，说明有新的连接请求
                /* 初始化client Address */
                struct sockaddr clientAddr;
                socklen_t size = sizeof(struct sockaddr);
                memset((char *)&clientAddr, 0, sizeof(struct sockaddr));
                /* 获取请求的connfd */
                connfd = accept(this->socket_fd, &clientAddr, &size);

                if( connfd == -1){
                    std::cerr << TAG_ERROR << "Accept error:" << strerror(errno) << std::endl;
                    continue;
                }

                /* 将新连接与其地址信息建立映射 */
                fdmap[connfd] = clientAddr;

                /* 定义connfd的evt对象 */
                evt.events = EPOLLIN;
                evt.data.fd = connfd;

                /* 使用epoll_ctl加入到epoll树中 */
                if(epoll_ctl(this->epfd, EPOLL_CTL_ADD, connfd, &evt) == -1)
                {
                    std::cerr << TAG_ERROR << "Epoll add error:" << strerror(errno) << std::endl;
                    return ;
                }
            }else
            { //对于除listenfd以外的fd,如果被返回，说明有数据可读
                connfd = evts[i].data.fd;
                /* 执行任务 */
                RunTask(connfd);

                
                /* 从epoll树中删除相应的connfd, 然后再close这个fd，切记不可先close该fd，否则会报错*/
                /* 当然根据任务的不同，也可以根据connection的状态来删除或者保留该fd */
                if(epoll_ctl(this->epfd, EPOLL_CTL_DEL, evts[i].data.fd, NULL) == -1){
                    std::cerr << TAG_ERROR << "Epoll add error:" << strerror(errno) << std::endl;
                    return ;
                }
...
*/
void myHttpServer::HttpServer::RunTask(int connectfd){    
