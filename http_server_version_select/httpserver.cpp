...
/* 没用上这个 */
void myHttpServer::HttpServer::sock_accept()
{
    while (1)
    {
...
/* 没用上这个 */
void* myHttpServer::HttpServer::call_sock_accept(void* pThis){
    
    // (HttpServer *)(pThis)->sock_accept();
    static_cast<HttpServer *>(pThis)->sock_accept();
}
...
void myHttpServer::HttpServer::ExecReqst()
{
    ...
    // 最初想法，另开个线程，来负责获取clifd，但是需要加锁，那还不如直接按顺序写了
    // pthread_create(&conn_thread_id, NULL, call_sock_accept, (void*)this);
